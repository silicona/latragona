	...
	envio.onreadystatechange = function(){
		...
		if( this.status == 200 && this.readyState == 4 ){
			...
			if( idioma == 'de' ){
				...
				var arr_respuesta = this.responseText.split("\n");
			...
			//imprimir(JSON.stringify(this.responseText));
			...
			if( salida.length < 22 ){
				...
				salida.unshift('<div>Faltan ' + (salida.length - 22) + ' elementos del archivo lexico_' + idioma + '.yml</div>');

			} else {

				salida.unshift('<div>Todos los elementos recibidos del archivo lexico_' + idioma + '.yml</div>');
			}
