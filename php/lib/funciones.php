...
function cambiaf_a_normal($fecha, $devuelve_blanco = ''){
	...
	// Con $devuelve_blanco y todo 0, devolvemos ''
...
// Test
function cambiaf_normal_a_larga($fecha){

	// Del formato dd/mm/aaaa al formato 'dia de mes de año'
	...
	$arr_meses = array('Enero', 'Febrero', 'Marzo', 'Abríl', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre');

	$fecha = $dia . ' de ' . $arr_meses[$mes] . ' de ' . $ano;
...
/*
// Test
function comprobar_hash($string, $hash, $url = false){
	// Generador de hashes
	
	$hasher = new PasswordHash(8, false);

	$hash = $url ? urldecode($hash) : $hash;

	$res = $hasher -> CheckPassword($string, $hash);
...
function devuelve_provincia($cp){
	...
	if( strlen($cp) == 5 ){	$cod_provincia = substr($cp, 0, 2); }

	if( strlen($cp) == 4 ){	$cod_provincia = '0' . substr($cp, 0, 1); }

	if( strlen($cp) == 2 ){	$cod_provincia = $cp; }
...
// Test
function eliminar_de_array($array, $elemento){
...
/*
// No Test - Prueba Live en Mantenimiento
function enviar_email($email_enviar_a, $nombre_enviar_a, $asunto, $html_mensaje, $archivos = '', $cc = '', $cco = ''){
	...
	if( INM_ENTORNO == 'pruebas' ){
		...
		// Enviar a Alejo
		$email_enviar_a  = 'aizquierdo@oclem.com';
		$nombre_enviar_a = 'Alejo';
	...
	try {
		...
		$mail -> Password   = '42KLJdasASD47FSAFSDF*';                              // SMTP password
...
function enviar_sms_altiria($sDestination, $sMessage, $debug = false){
	...
	*/
	...
	if( strlen($sDestination) != 12){
		...
		//print_r('Teléfono no válido');
		return false;
	}

	//print_r('telefono: ' . $sDestination);
...
// Test - Revisar para implementar $eng
...
/*
// Test
function generar_hash($string, $url = false){

	if( strlen($string) > 72 || $string == '' ){

		//die("El password es demasiado largo"); 
		return false;
	}
	...
	return $url ? urlencode($hash) : $hash;
}
*/
...
// Test - No se puede probar por el require_once
function i($mensaje = '', $etiqueta = ''){
	...
	if( INM_ENTORNO === 'pruebas' ){
		...
		*/
		...
		require_once BASE_FILE . 'php/lib/RES_FirePHP.php';
		return FB::info($mensaje,$etiqueta);
...
function limpiar_comillas($cadena){
	...
	//if( !preg_match('/^{.*}$/', $cadena) ){ return $cadena; }
...
// Test
function ordenar_array($array, $funcion = null){

	if( gettype($array) == 'object' ){ $array = object2array($array); }
	
	foreach( $array as $ind => $valor ){

		// Ordena por elemento o clave
		$nombre_funcion = is_numeric($ind) ? "usort" : "uksort";
		
		break;
	}


	$nombre_funcion( $array, function($a, $b){

		$val1 = mb_strtolower($a);
		$val2 = mb_strtolower($b);
...
function quitar_espacios($cadena){
	...
	$cadena = str_replace(['&nbsp;', "\xc2\xa0"], '', $cadena);
...
function quitar_ultimo_caracter($cadena){
	...
	if( $largo > 0 ){ $cadena = substr($cadena,0,($largo - 1)); }
...
function shorten_string($string, $wordsreturned){
	...
	// Returns the first $wordsreturned out of $string.  
	// If string contains fewer words than $wordsreturned, the entire string
	// is returned.
	...
	if(count($array) <= $wordsreturned ){
		...
		// Si es corto, devuelve todo el string
		$retval = $string;
...
// Test - Skipped - Error en .../plataforma/lib/PHPExcel/Calculation/Functions.php on line 581
